...
type MessageBoardServer struct {
	...
	// retry mehanizem za povezovanje na next node
	nextConnectMu     sync.Mutex
	nextConnectCancel context.CancelFunc
	nextConnectTarget string
...
func (s *MessageBoardServer) ConnectToNextNode(address string) {
	// prekliči morebitne stare retry goroutine
	s.nextConnectMu.Lock()
	if s.nextConnectCancel != nil {
		s.nextConnectCancel()
		s.nextConnectCancel = nil
	}
	s.nextConnectTarget = address
	ctx, cancel := context.WithCancel(context.Background())
	s.nextConnectCancel = cancel
	s.nextConnectMu.Unlock()

	// posebni primeri: izvedemo takoj, brez retry
	...
	// address=="" pomeni: postanemo tail (rep), next=nil
	if address == "" {
		s.mu.Lock()
		s.nextNode = nil
		s.IsTail = true
	...
	// normalni primer: poveži se v ozadju z retry, ker se drugi node morda še zaganja
	go s.connectToNextNodeWithRetry(ctx, address)
}

func (s *MessageBoardServer) connectToNextNodeWithRetry(ctx context.Context, address string) {
	...
	for {
		...
		if err != nil {
			log.Printf("[%s] connect to next (%s) failed: %v (retry in %s)", s.nodeId, address, err, backoff)
		...
		// začasno ustavimo replikacijo, dokler snapshot ne uspe
		s.mu.Lock()
		s.nextNode = nil
		s.IsTail = false
		s.mu.Unlock()
		...
		if err != nil {
			_ = conn.Close()
			log.Printf("[%s] snapshot to next (%s) failed: %v (retry in %s)", s.nodeId, address, err, backoff)
			if !sleepOrDone(ctx, backoff) {
				return
			}
			backoff = minDur(backoff*2, maxBackoff)
		...
		// snapshot uspe -> aktiviramo replikacijo
		s.mu.Lock()
		s.nextNode = client
		s.mu.Unlock()

		log.Printf("[%s] Connected to next node %s, snapshot installed, replication enabled", s.nodeId, address)

		// pomembno: conn naj ostane odprt (client ga rabi).
		// Če bi ga zaprl, bi nextNode client “umrl”.
		return
	}
}
