...
// pomozna funkcija iz cmd\server\main
...
// test kjer preverimo delovanje pri odpovedi glave. preden posljemo drugo sporocilo (po spremembi stanja vozlisc se vprasamo controlno ravnino za novo glavo)
func TestIntegrationReplicationWithHeadNodeFailure(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("PostMessageAfterReconfigure", func(t *testing.T) {
		...
		//dobimo novo glavo
		head, _, err := getClusterState(ctrlAddr)
		if err != nil {
		...
		//posljemo sporocilo v novo glavo
		msgResp, err = headClient.PostMessage(ctx, &razpravljalnica.PostMessageRequest{
			TopicId: topicResp.Id,
			UserId:  u.Id,
			Text:    "What about the 2nd breakfast?",
...
func TestIntegrationReplicationWithTailNodeFailure(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("PostMessageAfterReconfigure", func(t *testing.T) {
		...
		//dobimo nov rep
		_, tail, err := getClusterState(ctrlAddr)
		if err != nil {
			log.Fatalf("Failed to get cluster state: %v", err)
		}
		tailClient, connTail := connectToNode(tail.Address)
		defer connTail.Close()

		//posljemo sporocilo v glavo
		msgResp, err = headClient.PostMessage(ctx, &razpravljalnica.PostMessageRequest{
			TopicId: topicResp.Id,
			UserId:  u.Id,
			Text:    "What about the 2nd breakfast?",
